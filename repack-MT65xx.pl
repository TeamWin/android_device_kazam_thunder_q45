...
# adapted by Ethan Yonker aka Dees_Troy to support packing inline in a compile tree and lzma ramdisk
...
sub repack_boot {
	...
	foreach my $tool ("find", "cpio", "lzma") {
	...
	open (RAMDISKFILE, "ramdisk-repack.cpio.lzma") or die colored ("Error: could not open ramdisk file 'ramdisk-repack.cpio.lzma'", 'red') . "\n";
	my $ramdisk;
	while (<RAMDISKFILE>) {
		$ramdisk .= $_;
	}
	...
	open (RAMDISKFILE, ">new-ramdisk-repack.cpio.lzma");
	binmode (RAMDISKFILE);
	...
	if ( $^O eq "cygwin" ) {
		system ("$Bin/mkbootimg.exe --kernel $kernel --ramdisk new-ramdisk-repack.cpio.lzma -o $outfile");
	} else {
		system ("$ARGV[4] --kernel $kernel --ramdisk new-ramdisk-repack.cpio.lzma -o $outfile");
	}

	# cleanup
	unlink ("ramdisk-repack.cpio.lzma") or die $!;
	system ("rm new-ramdisk-repack.cpio.lzma");
